...
- **AI category audit is automatic**: `POST /ai-audit` (second opinion over all expense merchants, verdicts cached per (merchant, current) in-process) now runs as stage 3 of the background chain in `Layout.tsx`; the client auto-applies proposals with `confidence ≥ 0.85` AND `catalog_hit == false` (the catalog governs catalog-known merchants on every restore — the audit must not fight it) via `POST /merchants/category` + rule upsert. The manual review-queue UI was removed from ניהול נתונים per user request (no buttons — everything automatic).
