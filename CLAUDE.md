...
- **Subcategories** (`קטגוריה_משנה`): keyword-seeded via `SUBCATEGORY_KEYWORDS` (scoped per parent) in `data_processor.derive_subcategory()` (runs **after** user rules, fills only empties), plus manual assignment persisted as `user_category_rules.subcategory` (**run the idempotent `ALTER` in `supabase_setup.sql` once in Supabase**). Catalog exposed via `GET /api/categories/catalog`. **AI subcategory split**: `POST /ai-subcategorize` (session + category) groups the category's unsubcategorized merchants by canonical key and lets Claude reuse existing names or **create new subcategories** (e.g. `סופרים`); unknown merchants are web-searched, unclear ones stay empty, manual assignments are never clobbered; the frontend ("פילוח AI" button in the category drawer) persists assignments as merchant rules. `סופרים` is keyword-seeded FIRST under מזון וצריכה (so `יינות ביתן` isn't stolen by the `יין` keyword of אלכוהול ומשקאות).
